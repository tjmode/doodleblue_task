...
class Db:
	...
	def name_exists(self, name):
		...
		cursor.execute("select name from details where name=(%s)",[name])
	...
	def last_coupon_selecting(self):
		cursor = self.connection.cursor()
		cursor.execute("select coupon from details order by id desc limit 1")
		last_coupon=cursor.fetchall()
		return last_coupon
	def when_coupon_not_null(self):
		cursor = self.connection.cursor()
		cursor.execute("select coupon from details")
		refered_user=cursor.fetchall()
		return refered_user
	def registering(self,name,age,city,genrated_coupon):
		cursor = self.connection.cursor()
		cursor.execute("insert into details(name,age,city,coupon) values(%s,%s,%s,%s)",(name,age,city,genrated_coupon))
		self.connection.commit()
	def refernce_table(self,coupon,name):
		'''
		this condition is to insert the referal and refree in table seprate table
		here i can also use cur.execute("insert into refernce (referral)select name from details where coupon=(%s))",[coupon]) this is subquery 
		'''
		cursor = self.connection.cursor()
		cursor.execute("select name from details where coupon=(%s)",[coupon])
